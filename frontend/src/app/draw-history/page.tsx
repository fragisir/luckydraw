     ...
     getDrawHistory().then(res => {
       if(res.success) {
         ...
         if (res.history.length > 0) {
            ...
            const diff = new Date().getTime() - new Date(latest.drawDateTime).getTime();
            if (diff < 60000) { 
    ...
    <div className="bg-slate-50 min-h-screen py-10">
      <div className="container-official max-w-5xl mx-auto px-4 md:px-6">
           ...
           <div className="mb-12 animate-in fade-in zoom-in duration-500">
              ...
              {liveNotification.type === 'SALES_STOPPED' && (
                 <div className="bg-indigo-900 rounded-2xl shadow-2xl p-8 md:p-12 text-center border-b-8 border-indigo-600">
                     ...
                     <h2 className="text-3xl md:text-5xl font-black text-white mb-2">SALES CLOSED</h2>
                     <p className="text-indigo-200 text-xl">The draw is about to begin. Prepare for results!</p>
              ...
              {liveNotification.type === 'RESULT' && liveNotification.data && (
                  <div className="bg-slate-900 rounded-2xl shadow-2xl p-8 md:p-12 relative overflow-hidden text-center">
                     ...
                     <h2 className="text-3xl md:text-5xl font-black text-white mb-2 mt-4">{liveNotification.data.drawName}</h2>
                     <p className="text-slate-400 mb-8 font-mono">{new Date(liveNotification.data.drawDateTime).toLocaleString()}</p>
                     
                     <div className="flex flex-wrap justify-center gap-3 md:gap-4 mb-8">
                        {liveNotification.data.winningNumbers.map((num: number, i: number) => (
                           <div key={i} className="w-12 h-12 md:w-20 md:h-20 rounded-full bg-gradient-to-br from-amber-400 to-orange-600 text-white flex items-center justify-center text-xl md:text-4xl font-black shadow-lg border-4 border-white/10 animate-bounce" style={{animationDelay: `${i * 100}ms`}}>
                             {num}
                           </div>
                        ))}
                     </div>

                     <Link href={`/draw-history/${liveNotification.data.drawId}`} className="inline-block bg-white text-slate-900 px-8 py-3 rounded-full font-bold hover:bg-indigo-50 transition-colors">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-slate-800">Draw History</h1>
            <p className="text-slate-500">Official archive of all past results</p>
          </div>
          <Link href="/fairness-and-transparency" className="text-sm font-medium text-indigo-600 hover:underline">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 text-slate-400">No draws have been conducted yet.</div>
        ) : (
          <div className="space-y-4">
            {history.map((draw) => (
              <div key={draw._id} className="modern-card p-0 overflow-hidden flex flex-col md:flex-row shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="p-6 md:w-48 bg-slate-50 border-b md:border-b-0 md:border-r border-slate-100 flex flex-col justify-center text-center md:text-left">
                  ...
                  <p className="text-xl font-bold text-slate-800">
                    {new Date(draw.drawDateTime).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
                  </p>
                ...
                <div className="p-6 flex-1 flex flex-col justify-center items-center md:items-start border-b md:border-b-0 border-slate-100">
                  <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 flex items-center gap-2">
                    <Trophy size={14} className="text-amber-500" /> Winning Numbers
                  </h4>
                  <div className="flex flex-wrap justify-center gap-2">
                    {draw.winningNumbers.map((n: number) => (
                      <span key={n} className="w-10 h-10 rounded-full bg-indigo-600 text-white flex items-center justify-center font-bold shadow-sm border border-indigo-700">
                        {n}
                      </span>
                    ))}
                  </div>

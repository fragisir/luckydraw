...
export default function BuyTicket() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await createCheckoutSession({ name, email, selectedNumbers });
      ...
      const msg = err.response?.data?.message || err.message || 'System error occurred. Please try again.';
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-24 md:pb-12">
      ...
      <div className="bg-white border-b border-slate-200 p-4 md:p-8 text-center sticky top-0 z-10 shadow-sm md:static md:shadow-none">
         <h1 className="text-xl md:text-3xl font-bold text-slate-800">Choose Your Numbers</h1>
         <p className="text-sm md:text-base text-slate-500 mt-1">Select {MAX_Selection} lucky numbers (1-20)</p>
      </div>
        ...
        <div className="grid lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="modern-card p-6 md:p-10">
              <div className="grid grid-cols-5 gap-3 md:gap-6 justify-items-center">
                {Array.from({ length: 20 }, (_, i) => i + 1).map((num) => {
                  ...
                  const disabled = !isSelected && selectedNumbers.length >= MAX_Selection;
                  return (
                    <button
                      key={num}
            ...
            <div className="md:hidden modern-card p-4 bg-indigo-50 border-indigo-100">
               <p className="text-xs font-bold text-indigo-800 uppercase mb-2">Selected Numbers</p>
               <div className="flex justify-between">
                  {Array.from({ length: MAX_Selection }).map((_, i) => (
                    <div key={i} className={`
                      ...
                      ${selectedNumbers[i] ? 'border-indigo-500 text-indigo-700 shadow-sm' : 'border-slate-200 text-slate-300'}
                    `}>
                      {selectedNumbers[i] || '-'}
                    </div>
                  ))}
              ...
              <div className="hidden md:flex gap-2 mb-8 justify-center p-4 bg-slate-50 rounded-xl border border-slate-100">
                 {Array.from({ length: MAX_Selection }).map((_, i) => (
                    <div key={i} className={`
                      ...
                      ${selectedNumbers[i] ? 'border-indigo-500 text-indigo-700 shadow-sm' : 'border-dashed border-slate-200 text-slate-300'}
                    `}>
                      {selectedNumbers[i] || ''}
                    </div>
                  ))}
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="border-t border-slate-100 pt-6 mt-6">
                  <div className="flex justify-between items-end mb-2">
                    ...
                    <span className="text-3xl font-black text-slate-800 tracking-tight">$4.00</span>
                  </div>
                </div>
                ...
                <button 
                  type="submit"
                  disabled={loading || selectedNumbers.length !== MAX_Selection || !name || !email}
                  className="w-full btn-modern-primary py-4 text-lg shadow-xl"
                >
                  {loading ? <Loader2 className="animate-spin" /> : (
                    <span className="flex items-center">Purchase Ticket <Wallet className="ml-2 w-5 h-5" /></span>
                  )}
                </button>
              </form>
            </div>
